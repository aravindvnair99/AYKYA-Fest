$(document).ready(function () {
	...
	function count_down($area) {
		...
		let countDown = new Date("Sep 28, 2020 10:16:00").getTime(),
			x = setInterval(function () {
				...
				if (document.querySelector(".pagebanner__count-down-area")) {
					document.querySelector($area + "__day").innerText = prezero(
						dayy
					);
					document.querySelector(
						$area + "__hour"
					).innerText = prezero(hourr);
					document.querySelector($area + "__min").innerText = prezero(
						minn
					);
					document.querySelector($area + "__sec").innerText = prezero(
						secs
					);
				}
				if (distance <= 0) {
